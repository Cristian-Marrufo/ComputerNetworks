...
main(int argc, char* argv[])
{
    ...
    // MODIFICATION: Replaced single protocol with list to store multiple variants
    std::vector<std::string> transport_prot_list;
    std::string transport_prot_input;
    ...
    // MODIFICATION: Added outputDir to command line arguments, specify directory for output files
    cmd.AddValue("outputDir", "Directory where output files will be placed", outputDir);
    ...
    */
    ...
    // MODIFICATION: updated command line argument to accept multiple TCP variants
    cmd.AddValue("transport_prot", "Comma-separated list of TCP variants", transport_prot_input);
    ...
    std::stringstream ss(transport_prot_input);
    std::string proto;
    while(std::getline(ss, proto, ','))
        {
            transport_prot_list.push_back("ns3::" + proto);
        }
    if(transport_prot_list.size() != num_flows)
        {
            NS_FATAL_ERROR("Number of TCP variants (" << transport_prot_list.size() 
            << ") does not match number of flows (" << num_flows << ")");
        }
    ...
    if (!outputDir.empty()) {
        ...
        prefix_file_name = outputDir + prefix_file_name;
    }
    ...
    */
    ...
    // MODIFICATION: assign TCP variants to source node
    for (uint32_t i = 0; i < num_flows; i++)
    {
        Ptr<Node> node = sources.Get(i);
        Ptr<TcpL4Protocol> tcp = node->GetObject<TcpL4Protocol>();
        TypeId tid = TypeId::LookupByName(transport_prot_list[i]);
        tcp->SetAttribute("SocketType", TypeIdValue(tid));
    }
